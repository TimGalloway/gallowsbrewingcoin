...
const char * const BITCOIN_CONF_FILENAME = "gallowsbrewingcoin.conf";
...
#else
    const char* pszModule = "gallowsbrewingcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GallowsBrewingcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GallowsBrewingcoin";
#else
    // Unix
    return pathRet / ".gallowsbrewingcoin";
#endif
#endif
}
