# PSBT Howto for GallowsBrewingcoin Core

Since GallowsBrewingcoin Core 0.17, an RPC interface exists for Partially Signed GallowsBrewingcoin
...
PSBT is an interchange format for GallowsBrewingcoin transactions that are not fully signed
...
## PSBT in GallowsBrewingcoin Core
...
#### Multisig with multiple GallowsBrewingcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
GallowsBrewingcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `gallowsbrewingcoin-cli` is used.
