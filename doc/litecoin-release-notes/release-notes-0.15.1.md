GallowsBrewingcoin Core version *0.15.1* is now available from:

  <https://download.gallowsbrewingcoin.org/gallowsbrewingcoin-0.15.1/>
...
A number of changes to the way GallowsBrewingcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 brews per kB (0.00001 GBC). This is to prepare for dropping the minimum transaction fee to 0.1 brews per kB (0.0001 GBC) in the next release.
